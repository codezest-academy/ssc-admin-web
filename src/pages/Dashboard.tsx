...
import { Users, FileQuestion, GraduationCap, Activity, Loader2, ArrowUpRight } from "lucide-react";
...
export default function Dashboard() {
  ...
  if (isError || !data) {
    return (
      <div className="p-8 text-center text-destructive bg-destructive/10 rounded-xl border border-destructive/20">
        Failed to load dashboard data. Ensure the API is running and you have sufficient permissions.
      </div>
    );
  }
  ...
  const statCards = [
    { label: "Total Students", value: metrics.totalStudents, icon: Users, color: "text-blue-500", bg: "bg-blue-100" },
    { label: "Active Exams", value: metrics.activeExams, icon: GraduationCap, color: "text-emerald-500", bg: "bg-emerald-100" },
    { label: "Questions Bank", value: metrics.totalQuestions, icon: FileQuestion, color: "text-amber-500", bg: "bg-amber-100" },
    { label: "Total Test Attempts", value: metrics.totalAttempts, icon: Activity, color: "text-purple-500", bg: "bg-purple-100" },
  ];
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {statCards.map((stat) => (
          <div key={stat.label} className="rounded-xl border bg-card text-card-foreground shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        ...
        <div className="lg:col-span-4 rounded-xl border bg-card shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b bg-muted/20 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Latest submissions and ongoing tests.</p>
            </div>
          </div>
            ...
            {recentAttempts.length === 0 ? (
              ...
              <div className="divide-y">
                {recentAttempts.map((attempt) => (
                  <div key={attempt.id} className="p-4 flex items-center justify-between hover:bg-muted/30 transition-colors">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        <span className="font-medium text-sm text-foreground">{attempt.student.name}</span>
                        <Badge variant="outline" className="text-[10px] h-5 bg-background">
                          {attempt.attemptType}
                        </Badge>
                      </div>
                      <span className="text-xs text-muted-foreground">{attempt.student.email}</span>
                    </div>
                    
                    <div className="flex flex-col items-end gap-1">
                      {attempt.status === "SUBMITTED" ? (
                        <>
                          <Badge variant="default" className="bg-success/10 text-success hover:bg-success/20 border-none text-[10px] h-5">
                            Submitted
                          </Badge>
                          <span className="text-xs font-semibold text-slate-700">
                            {attempt.marksObtained !== null ? `${attempt.marksObtained} Marks` : "Evaluating..."}
                          </span>
                        </>
                      ) : (
                        <>
        ...
        <div className="lg:col-span-3 rounded-xl border bg-card shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b bg-muted/20 flex items-center justify-between">
            ...
            <Link to="/users" className="text-xs font-medium text-primary hover:underline flex items-center">
