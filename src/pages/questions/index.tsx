...
import { getQuestions, deleteQuestion, bulkImportQuestions } from "@/api/questions";
...
export default function QuestionsPage() {
  ...
  const handleBulkImport = async () => {
    try {
      ...
      toast.error(e.message || "Invalid JSON format or API error");
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage and curate questions for exams, sets, and tests.</p>
        </div>
          ...
          <Dialog open={isImportModalOpen} onOpenChange={setIsImportModalOpen}>
            ...
            <DialogContent className="max-w-2xl">
              ...
              <div className="py-4 space-y-4">
                <p className="text-sm text-muted-foreground">Paste a JSON array of questions matching the `CreateQuestionInput` schema.</p>
                <Textarea 
                  value={importJson} 
                  onChange={(e) => setImportJson(e.target.value)} 
                  placeholder={'[ { "subjectId": "...", "questionText": "..." } ]'} 
                  className="h-64 font-mono text-sm"
                />
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setIsImportModalOpen(false)}>Cancel</Button>
                <Button onClick={handleBulkImport} disabled={isImporting || !importJson.trim()}>
      ...
      <div className="border rounded-xl bg-card overflow-hidden">
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-8 text-muted-foreground">Loading questions...</TableCell>
              </TableRow>
            ) : questions.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-12">
                  <div className="flex flex-col items-center gap-2">
                    <p className="text-muted-foreground">No questions found in this bank.</p>
                    <Button variant="link" onClick={() => navigate("/questions/new")}>Add your first question</Button>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
              questions.map((q: any) => (
                <TableRow key={q.id}>
                  <TableCell>
                    <p className="text-sm font-medium line-clamp-2">{stripHtml(q.questionText)}</p>
                    <p className="text-xs text-muted-foreground mt-1">ID: {q.id}</p>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {q.isPYQ && <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20">PYQ {q.pyqYear}</Badge>}
