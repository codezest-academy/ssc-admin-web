...
import { getMockTestById, assignQuestionsToSection, createSection, deleteSection } from "@/api/mockTests";
...
import { ArrowLeft, Save, Loader2, GripVertical, Search, Plus, Trash2, FolderEdit, AlertTriangle, CheckCircle2 } from "lucide-react";
...
export default function MockTestBuilder() {
  ...
  useEffect(() => {
    if (activeSection?.questions) {
      const sorted = [...activeSection.questions].sort((a, b) => a.order - b.order);
  ...
  const saveQuestionsMutation = useMutation({
    mutationFn: () => assignQuestionsToSection(activeSectionId!, assignedQuestions.map(q => q.id)),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["mockTest", id] });
      setHasUnsavedChanges(false);
  ...
  // Derived Validation
  const currentTotalQuestions = sections.reduce((acc, s) => acc + (s.questions?.length || 0), 0);
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      ...
      <div className="flex items-center justify-between pb-4 border-b">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold tracking-tight">Builder: {mockTest.title}</h2>
            <div className="flex items-center gap-4 mt-1">
              <p className="text-sm text-muted-foreground">
                Blueprint: {mockTest.totalQuestions} Qs • {mockTest.totalMarks} Marks
              </p>
              
              <Badge variant={isValid ? "outline" : "secondary"} className={`text-xs ${isValid ? 'text-success border-success' : 'text-warning bg-warning/10'}`}>
      ...
      <div className="flex flex-1 gap-6 pt-4 min-h-0">
        ...
        <div className="w-64 flex flex-col border rounded-xl bg-card overflow-hidden shrink-0">
          <div className="p-4 border-b bg-muted/20 flex justify-between items-center">
            ...
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setIsCreatingSection(true)}>
            ...
            {isCreatingSection && (
              <div className="p-3 border rounded-lg bg-muted/30 space-y-3">
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => setIsCreatingSection(false)}>Cancel</Button>
                  <Button size="sm" className="h-7 text-xs" onClick={handleCreateSection} disabled={createSectionMutation.isPending}>Add</Button>
                </div>
              </div>
            )}
              ...
              <div 
                ...
                onClick={() => {
                  if (hasUnsavedChanges) {
                    if (!confirm("Discard unsaved changes in current section?")) return;
                  }
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <Button 
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      if (confirm("Delete this section and unassign all its questions?")) {
                ...
                <div className="text-xs text-muted-foreground mt-1 flex justify-between">
                  <span>{section.questions?.length || 0} / {section.questionCount} Qs</span>
            ...
            <div className="flex-1 flex gap-4 min-w-0">
              ...
              <div className="w-1/2 flex flex-col border rounded-xl bg-card border-primary/20 overflow-hidden relative">
                <div className="p-4 border-b bg-primary/5 flex justify-between items-center">
                  <div>
                    <h3 className="font-semibold text-primary">{activeSection.name}</h3>
                    <p className="text-xs text-muted-foreground mt-1">Target: {activeSection.questionCount} Qs • {activeSection.maxMarks} Marks</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <Badge variant={assignedQuestions.length === activeSection.questionCount ? "default" : "secondary"} className="text-xs">
                    ...
                    <Button 
                      ...
                      className="h-8"
                    >
                      {saveQuestionsMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4 mr-2" />} 
                      Save
                    </Button>
                  </div>
                </div>
                    ...
                    >
                      {assignedQuestions.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-muted-foreground border-2 border-dashed rounded-xl p-8">
                          ...
                          <p>Drag questions here to add to {activeSection.name}</p>
                        </div>
                      ) : (
