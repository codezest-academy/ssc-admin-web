...
export default function PracticeSetBuilder() {
  ...
  useEffect(() => {
    if (practiceSet?.questions) {
      ...
      const sorted = [...practiceSet.questions].sort(
        (a, b) => a.order - b.order,
      );
  ...
  const saveMutation = useMutation({
    mutationFn: (questionIds: string[]) =>
      assignQuestionsToSet(id!, questionIds),
    onSuccess: () => {
            ...
            <div className="p-4 border-b bg-primary/5 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Drag to reorder questions in the test.
                </p>
              </div>
